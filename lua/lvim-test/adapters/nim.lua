-- lvim-test.adapters.nim: the nim adapter (nimble).
-- nim's test command runs the whole suite (no reliable per-test CLI filter), so this adapter is
-- SUITE-granular: covered files are marked from the single run's exit code. `nimble` resolves through
-- lvim-lang.core.toolchain when the provider is active, else PATH.
--
---@module "lvim-test.adapters.nim"
...
local function bin(root)
    ...
    if ok and tc and tc.resolve then
        local r = tc.resolve("nim", "nimble", root)
        if r and r ~= "" then
            return r
        end
    end
    local p = vim.fn.exepath("nimble")
    return p ~= "" and p or "nimble"
end
...
local adapter = {
    ...
    toolchain_provider = "nim",

    ---@param path string
    ---@return boolean
    is_test_file = function(path, _root)
        return path:match("[/\\]tests?[/\\].*%%.nim$") ~= nil
    end,

    ---@param req LvimTestRunRequest
    ---@return LvimTestSpec?
    build = function(req)
        local a = config.adapters.nim or {}
    ...
    ---@param h table
    health = function(h)
        if vim.fn.exepath("nimble") == "" then
            h.warn("nimble not found on PATH")
        else
            h.ok("nimble: " .. vim.fn.exepath("nimble"))
        end
    end,
}
