-- lvim-test.adapters.roc: the roc adapter.
-- Roc runs inline expect-tests (`roc test`) per file — no per-test CLI filter. This adapter is SUITE-granular: covered files are marked from the single run's exit
-- code. `roc` resolves through lvim-lang.core.toolchain when the provider is active, else PATH.
--
---@module "lvim-test.adapters.roc"
...
local adapter = {
    ...
    filetypes = { "roc" },
    lang = "roc",
    root_markers = { "main.roc", ".git" },
    toolchain_provider = "roc",

    ---@param path string
    ---@return boolean
    is_test_file = function(path, _root)
        return path:match("%.roc$") ~= nil
    end,

    ---@param req LvimTestRunRequest
    ---@return LvimTestSpec?
    build = function(req)
        local a = config.adapters.roc or {}
    ...
    ---@param h table
    health = function(h)
        if vim.fn.exepath("roc") == "" then
            h.warn("roc not found on PATH")
        else
            h.ok("roc: " .. vim.fn.exepath("roc"))
        end
    end,
}
