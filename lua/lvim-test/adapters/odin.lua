-- lvim-test.adapters.odin: the odin adapter (odin).
-- odin's test command runs the whole suite (no reliable per-test CLI filter), so this adapter is
-- SUITE-granular: covered files are marked from the single run's exit code. `odin` resolves through
-- lvim-lang.core.toolchain when the provider is active, else PATH.
--
---@module "lvim-test.adapters.odin"
...
local adapter = {
    ...
    toolchain_provider = "odin",

    ---@param path string
    ---@return boolean
    is_test_file = function(path, _root)
        return path:match("%%.odin$") ~= nil
    end,

    ---@param req LvimTestRunRequest
    ---@return LvimTestSpec?
    build = function(req)
        local a = config.adapters.odin or {}
    ...
    ---@param h table
    health = function(h)
        if vim.fn.exepath("odin") == "" then
            h.warn("odin not found on PATH")
        else
            h.ok("odin: " .. vim.fn.exepath("odin"))
        end
    end,
}
