-- lvim-test.adapters.hare: the hare adapter (hare).
-- hare's test command runs the whole suite (no reliable per-test CLI filter), so this adapter is
-- SUITE-granular: covered files are marked from the single run's exit code. `hare` resolves through
-- lvim-lang.core.toolchain when the provider is active, else PATH.
--
---@module "lvim-test.adapters.hare"
...
local function bin(root)
    ...
    if ok and tc and tc.resolve then
        local r = tc.resolve("hare", "hare", root)
        if r and r ~= "" then
            return r
        end
    end
    local p = vim.fn.exepath("hare")
    return p ~= "" and p or "hare"
end
...
local adapter = {
    ...
    toolchain_provider = "hare",

    ---@param path string
    ---@return boolean
    is_test_file = function(path, _root)
        return path:match("%%.ha$") ~= nil
    end,

    ---@param req LvimTestRunRequest
    ---@return LvimTestSpec?
    build = function(req)
        local a = config.adapters.hare or {}
    ...
    ---@param h table
    health = function(h)
        if vim.fn.exepath("hare") == "" then
            h.warn("hare not found on PATH")
        else
            h.ok("hare: " .. vim.fn.exepath("hare"))
        end
    end,
}
